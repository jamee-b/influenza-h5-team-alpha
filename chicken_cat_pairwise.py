...
output_file = "chicken_cat_pairwise_alignment.txt"
...
cat_HA = [seq for id, seq in sequences.items() if any(x in id.lower() for x in ["cat"]) and "(HA)" in id]

# Debug: Check if we have sequences
print(f"Found {len(chicken_HA)} Chicken HA sequences.")
print(f"Found {len(cat_HA)} Cat HA sequences.")

if not chicken_HA or not cat_HA:
    ...
    # Compare each chicken HA with each cat HA
    for seq1, seq2 in product(chicken_HA, cat_HA):
