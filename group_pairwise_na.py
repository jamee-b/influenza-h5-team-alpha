...
output_file = "group_pairwise_alignment_na.txt"
...
bird_NA = [seq for id, seq in sequences.items() if any(x in id.lower() for x in ["chicken", "common grackle", "blackbird", "grackle"]) and "(NA)" in id]
mammal_NA = [seq for id, seq in sequences.items() if any(x in id.lower() for x in ["dairy cow", "cat", "feline", "cattle", "bovine"]) and "(NA)" in id]
human_NA = [seq for id, seq in sequences.items() if "viet nam" in id.lower() and "(NA)" in id]

# Debugging: Check if sequences are found
print(f"Found {len(bird_NA)} Bird NA sequences.")
print(f"Found {len(mammal_NA)} Mammal NA sequences.")
print(f"Found {len(human_NA)} Human NA sequences.")

# Exit if any group is empty
if not bird_NA or not mammal_NA or not human_NA:
...
with open(output_file, "w") as out_f:
    ...
    for seq1, seq2 in product(bird_NA, mammal_NA):
        alignments = aligner.align(seq1, seq2)
    ...
    if bird_NA and mammal_NA and human_NA:
        conserved_bird_mammal_human = find_conserved_regions(bird_NA[0], mammal_NA[0], human_NA[0])  # Using the first sequence from each group
