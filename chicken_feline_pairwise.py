...
output_file = "chicken_feline_pairwise_alignment.txt"
...
feline_HA = [seq for id, seq in sequences.items() if any(x in id.lower() for x in ["feline"]) and "(HA)" in id]

# Debug: Check if we have sequences
print(f"Found {len(chicken_HA)} Chicken HA sequences.")
print(f"Found {len(feline_HA)} Feline HA sequences.")

if not chicken_HA or not feline_HA:
    ...
    # Compare each chicken HA with each feline HA
    for seq1, seq2 in product(chicken_HA, feline_HA):
